// =============================================================================
// /api/admin/faq/[id] — update + delete a single FAQ entry.
//
...
import { updateFaq, softDeleteFaq, hardDeleteFaq } from '@/services';
...
async function handleUpdate(request: Request, id: string, locals: App.Locals) {
  ...
  const result = contentType.includes('application/json')
    ? parseJsonBody(await request.json(), faqSchema.partial())
    : parseFormData(await request.formData(), faqSchema);
  ...
  if (!result.success) {
    ...
    return redirectAbsolute(request, `/admin/faq/${id}/edit?${params.toString()}`, 303);
  }

  try {
    const faq = await updateFaq(locals.supabase, id, result.data as Parameters<typeof updateFaq>[2], locals.user);
    if (contentType.includes('application/json')) return jsonOk(faq);
    return redirectAbsolute(request, 
      `/admin/faq?toast=${encodeURIComponent(`Pertanyaan "${faq.question}" berhasil diperbarui.`)}&toastType=success`,
      303
    );
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Gagal memperbarui FAQ.';
    if (contentType.includes('application/json')) return jsonError(message, 500);
    return redirectAbsolute(request, `/admin/faq/${id}/edit?toast=${encodeURIComponent(message)}&toastType=error`, 303);
  }
}
...
async function handleSoftDelete(request: Request, id: string, locals: App.Locals, wantsJson: boolean) {
  ...
  try {
    await softDeleteFaq(locals.supabase, id, locals.user);
    if (wantsJson) return jsonOk({ deleted: true });
    return redirectAbsolute(request, `/admin/faq?toast=${encodeURIComponent('FAQ dipindahkan ke arsip.')}&toastType=success`, 303);
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Gagal menghapus FAQ.';
    if (wantsJson) return jsonError(message, 500);
    return redirectAbsolute(request, `/admin/faq?toast=${encodeURIComponent(message)}&toastType=error`, 303);
  }
}
...
async function handlePermanentDelete(request: Request, id: string, locals: App.Locals, wantsJson: boolean) {
  ...
  try {
    await hardDeleteFaq(locals.supabase, id, locals.user);
    if (wantsJson) return jsonOk({ deleted: true, permanent: true });
    return redirectAbsolute(request, `/admin/faq/trash?toast=${encodeURIComponent('FAQ dihapus permanen.')}&toastType=success`, 303);
  } catch (err) {
    const message = err instanceof Error ? err.message : 'Gagal menghapus permanen FAQ.';
    if (wantsJson) return jsonError(message, 500);
